    ...
    def __init__(self, dataset, model_class, config_name, target_variables, 
        ...
        if config_path:
            ...
            if self.config_name in search_spaces:
                ...
                self.space.append(self.get_batch_space()) #要使得search space变化就更改这里
            else:
                raise ValueError(f"'{self.config_name}' not found in the default config.")

    def get_batch_space(self, min_size = 128, max_size = 128):
    ...
    def perform_tuning(self, hpo_patience = 0):
        ...
        with tqdm(total=self.n_iter, desc='Tuning Progress') as pbar:
            for i in range(self.n_iter):
                ...
                record = suggested_params_dict.copy()
                record['loss'] = loss
                record['iter'] = i + 1
        ...
        configs_df = pd.DataFrame(all_configs).sort_values('iter').reset_index(drop=True)
        return best_model, best_params_dict, configs_df
...
class FineTuner(pl.LightningModule):
    ...
    """
    def __init__(self, model, dataset, n_splits=5, batch_size=128, learning_rates=None, max_epoch = 50, freeze_configs = None):
        super().__init__()
...
class LiveLossPlot(Callback):
    ...
    def plot_losses(self):
        ...
        epochs_to_show = 500  # Number of most recent epochs to display
