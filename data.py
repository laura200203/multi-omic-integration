...
class DataImporter:
    ...
    def import_data(self):
        ...
        # 增加新代码
        combined_dat = {k: pd.concat([train_dat[k], test_dat[k]], axis=1) for k in train_dat.keys()}
        combined_ann = pd.concat([train_ann, test_ann], axis=0)
        combined_samples = train_samples + test_samples
        import random; random.shuffle(combined_samples)
        whole_dataset = self.get_torch_dataset(combined_dat, combined_ann, combined_samples)
        print("[INFO] Combined (Whole) Dataset Stats: ", whole_dataset.get_dataset_stats())
        
        return training_dataset, testing_dataset, whole_dataset
    ...
    def read_data(self, folder_path):
        #读取必须的文件，储存到字典data里面。比如clin.csv 的key是clin, 值是对应的dataframe
        data = {}
        required_files = {'clin.csv'} | {f"{dt}.csv" for dt in self.data_types}
        print("\n[INFO] ----------------- Reading Data ----------------- ")
    ...
    def filter_by_features(self, dat, features):
        #在进行了前面的清洗过后可能需要删掉了一些特征，这个函数就是得到feature cleaning之后的数据集
        """
    ...
    def process_data(self, data, split = 'train'):
        #清理样本并对齐，选择train 中 top_percentile
    ...
    def normalize_data(self, data, scaler_type="standard", fit=True):
        ...
        if scaler_type is None:
            print("Skipping normalization as scaler_type is None")
            normalized_data = data  # 直接返回原始数据，变量名保持不变
        else:
            if fit:
                if scaler_type == "standard":
                    ...
                    raise ValueError("Invalid scaler_type. Choose 'standard', 'min_max' or None.")

            normalized_data = {
                x: pd.DataFrame(
                self.scalers[x].transform(data[x].T),
                index=data[x].columns,
                columns=data[x].index
                ).T for x in data.keys()
            }

        return normalized_data
    # 在这里加一条将train和test合并成一个整体，用来做模型特征选择的训练后续    
    ...
    def encode_labels(self, df):
        ...
        # 返回值是转换好的标签（数字），每列是数值还是类别， 原始标签（用来还原）
    ...
    def validate_input_data(self, train_dat, test_dat):   
     ...
     # 经过以上步骤，会得到一个dataframe,里面包括不同的组学的数据集的train和test       

class MultiOmicDataset(Dataset):
    # 用在dataimporter中，转化成不同模型需要的输入形式
